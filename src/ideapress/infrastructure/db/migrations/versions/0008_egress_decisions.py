...
Row J1 (IP-A2), and the **second** package-table mount in this application. ``0007`` was the
first-instance example (ADR-0050); this revision is its transcription, so the reasoning is there and
...
``ideapress.infrastructure.db.models``. As with the ledger: the package ships table *shapes* and
never a migration history, so this table appears in IdeaPress's own autogenerate diff, upgrades with
IdeaPress's own history, and is backed up, restored and pruned by whoever owns this database.

**Why this table is append-only in practice and has no delete path here.** A row is the audit record
of an egress verdict, and spec §11 contract 1 makes a denial exactly as durable as an approval.
Nothing in this application updates or deletes one; retention is the operator's, through the
database they own.
...
projections of fields inside it, indexed so a badge and a provenance view can filter without opening
