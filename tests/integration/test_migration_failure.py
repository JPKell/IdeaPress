...
``test_failed_migration_restores_backup_on_sqlite`` prove; this is that test for IdeaPress,
...
def _head(tmp_path: Path) -> str:
    """The single head of IdeaPress's own linear history."""
    ...
    try:
        ...
        engine.dispose()
    assert len(heads) == 1, f"IdeaPress's history must stay linear; found heads {heads}"
    return heads[0]
...
def test_a_failing_migration_restores_the_backup_and_keeps_existing_rows(
    ...
    head = _head(tmp_path)
    engine = create_engine_for(f"sqlite:///{tmp_path / 'ideapress.sqlite3'}")
    try:
        ...
        with engine.connect() as connection:
            connection.execute(
                ...
                {"k": "theme", "v": '"dark"', "u": "2026-08-29T00:00:00"},
            )
            connection.commit()
        ...
        assert [row[0] for row in rows] == ["theme"]
    finally:
        engine.dispose()
