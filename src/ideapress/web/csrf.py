"""ideapress.web.csrf — the page side of MirrorWall's double-submit CSRF (ADR-0026 §2).

MirrorWall's ``CsrfMiddleware`` rejects a form post whose ``csrf_token`` field does not equal the
``__Host-mw-csrf`` cookie. This module is the other half: a page carrying a form is rendered with a
token in a hidden field and the same token set as the cookie. The token is reused when the browser
already holds one, so two tabs do not invalidate each other's forms.

The ``__Host-`` prefix requires ``Secure``; browsers treat ``http://localhost`` and
``http://127.0.0.1`` as secure contexts, which is exactly the loopback bind IdeaPress defaults to.
