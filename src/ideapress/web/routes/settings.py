...
``403 FORBIDDEN`` naming it; an unknown key or a refused value is ``400`` naming it; a request
...
def put_settings(request: Request, body: Annotated[dict[str, Any], Body()]) -> JSONResponse:
    ...
    Raises:
        ...
        ValidationError: An unknown key, or a value its field refuses, named (``400``). Nothing is
            written.
    """
    runtime = request.app.state.runtime
